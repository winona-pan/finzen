            ...
            <Card style={{ padding:20, marginBottom:16 }}>
              ...
              {!collapsed["manual"] && [{
                ...
                steps:[
                  ...
                  "左滑記錄可刪除、右滑可編輯；刪除都會先跳出確認，刪除後畫面下方 6 秒內可以按「復原」救回來",
                ...
                steps:[
                  ...
                  "點帳戶名稱 → 查看該帳戶所有交易細項；右滑帳戶列可拉出編輯/刪除",
                  "帳戶順序：進入排序模式後用 ▲▼ 調整順序",
                  "子帳戶：金融卡帳戶下可以開子帳戶（願望、旅費、存錢等），直接顯示在錢包列表不用點進去。點名字可改名、點圖示可換emoji、點眼睛圖示可切換隱藏——隱藏的子帳戶金額不會算進總資產（適合薪水預支這類不算自己的錢）",
                  "信用卡：輸入「應付金額」，點 Pay 繳費時選扣款帳戶",
                  "訂閱/基本開銷：設定固定扣款頻率，新增當下就會立刻記一筆本期扣款，之後到期自動記帳",
                  "訂閱若選「每年」扣款，可開「年繳分攤認列」：扣款當下不整筆算進支出，改成每月自動認列 1/12",
                ]
              },{
                ...
                steps:[
                  ...
                  "個股詳細頁「交易紀錄」：列出每一筆買賣，點一筆可以編輯股數/單價，左滑可以刪除（編輯或刪除都不會自動調整帳戶餘額，需要的話自己到錢包調）",
                  "個股詳細頁可以設「停損%」「產業別」，達到停損會顯示警示；產業別填了會出現在持股頁的產業分佈圖",
                  "「績效」分頁：勝率/賺賠比/R值、最大回撤、與大盤(0050)比較、股息估算，都要點「重新整理」才會抓即時資料",
                  "「自選股」分頁：還沒買、想追蹤的股票，輸入代號加入，點一支可以展開股價走勢圖（1日～1年切換）",
                  "「持股」分頁最下面：全部交易紀錄、每日損益熱力圖、產業分佈圖",
                ...
                steps:[
                  "本頁月份切換是獨立的，跟總覽頁的月份互不影響",
                  "圓餅圖：查看本月各類別支出/收入佔比（代墊只算自己那份，往來帳收款不算收入）",
                  "收支健康度：自訂區間查看儲蓄率、支出佔比、訂閱月費",
                  "資產成長：可切換「資產水位」（Y軸會依你的資產範圍動態縮放）跟「每期變動」（本月只會畫到今天，不會顯示未來空白日期）",
                  "資產配置圖：改成依帳戶類型（現金/金融卡/證券帳戶）呈現",
                  "目標管理：點「＋新增目標」設定金額、期限、追蹤帳戶或子帳戶；30 天內到期變橘色；點📍可以釘選在總覽頁顯示",
                ...
                steps:[
                  ...
                  "Q：刪錯東西了怎麼辦？→ 刪除後畫面下方 6 秒內會有「復原」按鈕",
                  "小技巧：支出綠色、收入紅色（台灣股市漲紅跌綠慣例）",
                  "小技巧：帳戶餘額調整時填說明，之後看細項才知道為何調整",
                  "小技巧：股票、交易紀錄的編輯/刪除都不會自動連動帳戶餘額，需要的話記得自己到錢包調整",
                ]
              }].map((item) => (

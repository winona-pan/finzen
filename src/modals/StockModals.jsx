...
export default function StockModals({ 
  ...
  return (
    <>
        {modal === "buyStock" && <Sheet title={tr("記錄買入")} onClose={close}>
          <Sl label={tr("證券帳戶")} value={buyF.acc} onChange={e => setBuyF(p => ({ ...p, acc:e.target.value }))}><option value="">— {tr("選擇")} —</option>{accs.filter(a => a.type === "investment").map(a => <option key={a.id} value={a.name}>{a.name}</option>)}</Sl>
          
          {/* 記憶上次買入 */}
          {stocks.length > 0 && <div style={{ marginBottom:10 }}>
            <div style={{ fontSize:11, color:C.muted, marginBottom:6 }}>📌 {tr("重複買入（點選帶入）")}</div>
          ...
          <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:8 }}>
            <div>
              <Inp label={tr("股票代號")} placeholder="0050 / AAPL" value={buyF.ticker} onChange={e => setBuyF(p => ({ ...p, ticker:e.target.value.toUpperCase().replace(/\.TW$|\.US$/i,"") }))} />
              {buyF.ticker.includes(".") && <div style={{ fontSize:11, color:C.warn, marginTop:3 }}>⚠️ {tr("不需要加 .TW 或 .US")}</div>}
            </div>
            <Inp label={tr("股票名稱")} placeholder="元大台灣50" value={buyF.name} onChange={e => setBuyF(p => ({ ...p, name:e.target.value }))} />
          </div>
          <Sl label={tr("市場")} value={buyF.market} onChange={e => setBuyF(p => ({ ...p, market:e.target.value }))}><option value="TW">{tr("台股 TW")}</option><option value="US">{tr("美股 US")}</option></Sl>
          <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:8 }}>
            <Inp label={tr("股數")} type="number" placeholder="1000" value={buyF.shares} onChange={e => setBuyF(p => ({ ...p, shares:e.target.value, totalCost:p.avgCost?String(Math.round(+e.target.value*+p.avgCost+(+p.fee||0))):p.totalCost }))} />
          ...
          {isOverConcentrated && <div style={{ padding:12, borderRadius:12, marginBottom:4, background:`${C.warn}15`, border:`1px solid ${C.warn}55` }}>
            <div style={{ fontSize:12, fontWeight:900, color:C.warn }}>⚠️ {tr("這筆會佔總資產約")} {posPct.toFixed(1)}%，{tr("部位偏重，注意分散風險")}</div>
          </div>}
        ...
        {modal === "sellStock" && (() => {
          ...
          return <Sheet title={tr("賣出股票")} onClose={close}>
            <Fld label={tr("選擇持股")}>
              <select value={sellF.stockId} onChange={e => {
                ...
                {stSum.filter(s => s.totalSh > 0).map(s => (
                  <option key={s.id} value={s.id}>{s.ticker} {s.name} · {s.totalSh}{tr("股")} · {s.acc}</option>
                ))}
              </select>
            </Fld>
            ...
            <Fld label={tr("損益記錄（手動）")}>
              <div style={{ display:"flex", gap:8, marginBottom:6 }}>
                <button onClick={() => setSellF(p => ({ ...p, pnlType:"income" }))} style={{ flex:1, padding:"8px", borderRadius:10, fontWeight:700, fontSize:13, background:sellF.pnlType==="income"?`${C.income}28`:C.card, color:sellF.pnlType==="income"?C.income:C.muted, border:`1px solid ${sellF.pnlType==="income"?C.income:C.border}`, cursor:"pointer" }}>📈 {tr("獲利")}</button>
                <button onClick={() => setSellF(p => ({ ...p, pnlType:"expense" }))} style={{ flex:1, padding:"8px", borderRadius:10, fontWeight:700, fontSize:13, background:sellF.pnlType==="expense"?`${C.expense}28`:C.card, color:sellF.pnlType==="expense"?C.expense:C.muted, border:`1px solid ${sellF.pnlType==="expense"?C.expense:C.border}`, cursor:"pointer" }}>📉 {tr("虧損")}</button>
              </div>
              <CalcInp label={tr("損益金額")} value={sellF.pnl} onChange={v => setSellF(p => ({ ...p, pnl:v }))} />
              <div style={{ fontSize:11, color:C.muted, marginTop:4 }}>{tr("填入後會自動在總覽產生收支記錄")}</div>
            </Fld>
            <Sl label={tr("款項回流帳戶")} value={sellF.returnAcc} onChange={e => setSellF(p => ({ ...p, returnAcc:e.target.value }))}><option value="">— {tr("選擇")} —</option>{accs.filter(a => a.type !== "credit").map(a => <option key={a.id} value={a.name}>{AT[a.type] || ""} {a.name}</option>)}</Sl>
            <Fld label={tr("這筆是什麼心態下賣的？（選填，事後回顧用）")}>
              <div style={{ display:"flex", flexWrap:"wrap", gap:6 }}>
            ...
            <div style={{ display:"flex", gap:8, marginTop:8 }}>
              <Btn style={{ flex:1 }} onClick={doSell}>{tr("確認賣出")}</Btn>
        ...
        {modal === "stockDetail" && selStock && (() => {
          ...
          return <Sheet title={`${st.ticker} ${st.name}`} onClose={close}>
            <Card style={{ padding:16, marginBottom:12, background:`linear-gradient(135deg,${C.surface},${C.bg})` }} onClick={() => hideAmounts && doPeek()}>
              ...
              {hasPrice && <div style={{ display:"flex", justifyContent:"space-between", padding:"8px 0", borderTop:`1px solid ${C.border}`, cursor:hideAmounts?"pointer":"default" }}>
                ...
                <div style={{ textAlign:"right" }}>
                  ...
                  <div style={{ fontWeight:900, fontSize:16, color:pnlColor(pnl, C), ...maskStyle }}>{pnl >= 0 ? "▲ +" : "▼ "}{fmt(Math.abs(pnl))}</div>
                  <div style={{ fontSize:11, fontWeight:700, color:pnlColor(pnl, C), ...maskStyle }}>{pnlPct >= 0 ? "+" : ""}{pnlPct.toFixed(2)}%</div>
                </div>
              </div>}
              {st.lastUpdated && <div style={{ fontSize:10, color:C.muted, marginTop:4 }}>{tr("更新")}：{st.lastUpdated}</div>}
            ...
            {hasPrice && (extra.high || extra.low || extra.vol) && <Card style={{ padding:14, marginBottom:12 }}>
              <div style={{ fontSize:11, fontWeight:900, color:C.muted, marginBottom:8, letterSpacing:"0.08em" }}>{tr("今日行情")}</div>
              <div style={{ display:"grid", gridTemplateColumns:"repeat(4,1fr)", gap:8 }}>
                {[{l:tr("最高"), v:extra.high, c:C.income}, {l:tr("最低"), v:extra.low, c:C.expense},
                  {l:tr("成交量"), v:extra.vol ? (extra.vol>1000?`${(extra.vol/1000).toFixed(0)}K`:extra.vol) : null, c:C.text},
                  {l:tr("漲跌幅"), v:extra.chgPct!==undefined?`${extra.chgPct>=0?"+":""}${extra.chgPct}%`:null, c:pnlColor(extra.chgPct||0,C)}
            ...
            <Inp label={tr("投資總成本（自動）")} type="number" value={String(Math.round(st.totalCost||0))}
            ...
            <div style={{ padding:12, borderRadius:12, background:`${C.danger}10`, border:`1px solid ${C.danger}33`, margin:"10px 0" }}>
              <div style={{ fontSize:11, fontWeight:900, color:C.danger, marginBottom:8 }}>🔴 {tr("停損提醒")}</div>
              <div style={{ display:"flex", alignItems:"center", gap:8 }}>
                <span style={{ fontSize:13, color:C.textSub, flexShrink:0 }}>{tr("虧損超過")}</span>
                ...
                <span style={{ fontSize:13, color:C.textSub, flexShrink:0 }}>% {tr("時顯示警示")}</span>
            ...
            {totAssets > 0 && (() => {
              ...
              return (
                <div style={{ padding:10, borderRadius:10, background:posPct > 20 ? `${C.warn}15` : `${C.card}`, border:`1px solid ${posPct > 20 ? C.warn+"55" : C.border}`, marginBottom:4 }}>
                  <div style={{ fontSize:11, color:C.textSub }}>{tr("這檔佔總資產比重")}</div>
                  <div style={{ fontSize:14, fontWeight:900, color:posPct > 20 ? C.warn : C.text }}>{posPct.toFixed(1)}%{posPct > 20 ? `　⚠️ ${tr("部位偏重，注意集中風險")}` : ""}</div>
                </div>
              );
            })()}
            ...
            {(st.trades||[]).length > 0 && (() => {
              ...
              return <div style={{ margin:"14px 0" }}>
                <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:4 }}>
                  <div style={{ fontSize:11, fontWeight:900, textTransform:"uppercase", letterSpacing:"0.08em", color:C.muted }}>{tr("交易紀錄（共")} {st.trades.length} {tr("筆）")}</div>
                ...
                <div style={{ fontSize:10, color:C.muted, marginBottom:8 }}>{tr("點一筆可以編輯股數/單價；編輯或刪除都不會自動調整帳戶餘額，需要的話請自行到錢包調整")}</div>
                  ...
                  {editingTrade === t.id ? (
                    <div style={{ padding:"10px 4px", borderTop:i>0?`1px solid ${C.border}`:undefined, background:`${C.accent}08` }}>
                      ...
                      <div style={{ display:"flex", gap:6 }}>
                        ...
                        <button onClick={() => setEditingTrade(null)} style={{ padding:"8px 16px", borderRadius:8, background:C.card, color:C.text, border:`1px solid ${C.border}`, fontWeight:700, fontSize:12, cursor:"pointer" }}>{tr("取消")}</button>
                      </div>
                    </div>
                  ) : (
            ...
            <Btn v="danger" style={{ width:"100%", marginTop:8 }} onClick={() => confirm(`${tr("確定刪除")} ${st.ticker}？（${tr("不會自動退回帳戶餘額，需要的話請自行到錢包調整")}）`,()=>{upd("stocks",p=>p.filter(s=>s.id!==st.id));close();})}>🗑 {tr("刪除此持股")}</Btn>
          </Sheet>;
        })()}
    </>
  );

...
const THEMES = {
  dark: {
    bg:"#0d0f14", surface:"#14161e", card:"#1a1d28",import { useState, useMemo, useRef, useCallback, useEffect } from "react";
import { AreaChart, Area, LineChart, Line, PieChart, Pie, Cell, XAxis, YAxis, CartesianGrid, Tooltip, ResponsiveContainer, Legend } from "recharts";const THEMES = {
  dark: { bg:"#0d0f14", surface:"#14161e", card:"#1a1d28", bordeimport { useState, useMemo, useRef, useCallback, useEffect } from "react";
import { AreaChart, Area, LineChart, Line, PieChart, Pie, Cell, XAxis, YAxis, CartesianGrid, Tooltip, ResponsiveContainer, Legend } from "recharts";
            ...
            <div style={{ padding:"12px 16px" }}>
              ...
              {invTab === "holdings" && <div>
                <Card style={{ padding:20, marginBottom:16, background:`linear-gradient(135deg,${C.surface},${C.bg})` }}>
                  ...
                  {stTotMv > 0 && stTotCost > 0 && <div style={{ display:"flex", justifyContent:"space-between", padding:"10px 0", borderTop:`1px solid ${C.border}`, marginBottom:12 }}>
                    <div>
                  ...
                  return <div key={accN} style={{ marginBottom:16 }}>
                    ...
                    {!isCollapsed && <Card style={{ overflow:"hidden" }}>
                      {stks.map((st, i) => {
                        ...
                        // Ticker key matching for prices.json
                        const priceKey = st.market === "TW" ? st.ticker : st.ticker;
                        return <SwipeRow key={st.id}
                          onDelete={() => confirm(`確定刪除 ${st.ticker}？`, () => upd("stocks", p => p.filter(s => s.id !== st.id)))}
                          onEdit={() => { setSelStock(st); setModal("stockDetail"); }}
                          onClick={() => { setSelStock(st); setModal("stockDetail"); }}>
                          <div style={{ padding:"12px 16px", borderTop:i > 0 ? `1px solid ${C.border}` : undefined }}>
                            ...
                            <div style={{ display:"flex", justifyContent:"space-between", fontSize:11, color:C.muted }}>
                              ...
                              {hasPrice
                                ? <span style={{ color:C.textSub }}>市價 {fmt(st.curPrice)}{st.lastUpdated ? ` · ${st.lastUpdated}` : ""}</span>
                                : <span>成本 {fmt(st.totalCost)}</span>}
                            </div>
                          </div>
                        </SwipeRow>;
                      })}
...
}r:"#252839", borderL:"#303550", income:"#f43f5e", expense:"#4ade80",
    accent:"#7c7cf8", accentL:"#a5b4fc", accentD:"#5b5bd6", warn:"#fb923c", teal:"#2dd4bf",
    text:"#eef0fa", textSub:"#7c80a0", muted:"#444660", danger:"#ef4444", name:"深色", icon:"🌙", },
  light: { bg:"#f4f6fc", surface:"#ffffff", card:"#ffffff", border:"#e2e8f0", borderL:"#cbd5e1", income:"#e11d48", expense:"#16a34a",
    accent:"#6366f1", accentL:"#4f46e5", accentD:"#4338ca", warn:"#d97706", teal:"#0d9488",
    text:"#1e293b", textSub:"#475569", muted:"#94a3b8", danger:"#dc2626", name:"淺色", icon:"☀️", },
  purple: { bg:"#0e0b1a", surface:"#16112b", card:"#1f1840", border:"#2e2550", borderL:"#3d3370", income:"#f43f5e", expense:"#34d399",
    accent:"#a855f7", accentL:"#c084fc", accentD:"#9333ea", warn:"#f59e0b", teal:"#06b6d4",
    text:"#f3e8ff", textSub:"#a78bfa", muted:"#6d5a9e", danger:"#ef4444", name:"紫色", icon:"💜", },
  ocean: { bg:"#020b18", surface:"#061825", card:"#0a2236", border:"#0f3450", borderL:"#1a4a6e", income:"#f43f5e", expense:"#4ade80",
    accent:"#0ea5e9", accentL:"#38bdf8", accentD:"#0284c7", warn:"#f59e0b", teal:"#14b8a6",
    text:"#e0f2fe", textSub:"#7dd3fc", muted:"#2d6a8a", danger:"#ef4444", name:"海洋", icon:"🌊", },
};
let C = THEMES.dark;
function getC(theme) { return THEMES[theme] || THEMES.dark; }
const PIE = ["#f43f5e","#7c7cf8","#4ade80","#fb923c","#06b6d4","#ec4899","#a78bfa","#34d399"];
const DAYS = ["Sun","Mon","Tue","Wed","Thu","Fri","Sat"];
const TODAY = new Date(new Date().getTime() + 8 * 60 * 60 * 1000).toISOString().slice(0, 10);const DEF_RATES = { TWD:1,USD:32.5,EUR:35.2,JPY:0.22,GBP:41.0,HKD:4.17,SGD:24.1,CNY:4.48,KRW:0.024,AUD:21.0,CAD:23.8,CHF:36.5,MYR:7.3,THB:0.93,VND:0.0013 };
const CUR_SYM = { TWD:"NT$",USD:"$",EUR:"€",JPY:"¥",GBP:"£",HKD:"HK$",SGD:"S$",CNY:"¥",KRW:"₩",AUD:"A$",CAD:"C$",CHF:"CHF",MYR:"RM",THB:"฿",VND:"₫" };
const CUR_NAME = { TWD:"新台幣",USD:"美金",EUR:"歐元",JPY:"日圓",GBP:"英鎊",HKD:"港幣",SGD:"新加坡幣",CNY:"人民幣",KRW:"韓元",AUD:"澳幣",CAD:"加幣",CHF:"瑞士法郎",MYR:"馬幣",THB:"泰銖",VND:"越南盾" };
const ALL_CURS = Object.keys(DEF_RATES);
function toTWD(n, cur, rates) { return n * (rates[cur] || 1); }
...
const APP_VER = "2.2";
const DATA_KEY = "finzen_v3";
const VER_KEY = "finzen_ver";const DEF = {
  accs: [
    { id:"a1",name:"現金",   type:"cash",   cur:"TWD",bal:0,vis:true,order:0 },
    { id:"a2",name:"銀行帳戶",type:"debit",  cur:"TWD",bal:0,vis:true,order:1 },
    { id:"c1",name:"信用卡", type:"credit",  cur:"TWD",bal:0,payable:0,limit:100000,vis:true,order:2 }, ],
  txns:[], debts:[], subs:[], bills:[], stocks:[], pools:[], customCE: {}, goals: [],
  policies: [], // 儲蓄險/投資型保單
  cats: {
    expense: ["食物","交通","家居","娛樂","訂閱","教育","醫療","美容","保費","其他","往來帳"],
    income:  ["薪資","家教","零用錢","利息","股息","紅包","投資收益","其他收入","往來帳"], }, rates: DEF_RATES,
};function loadData() {
  try {
    const s = localStorage.getItem(DATA_KEY);
    if (!s) return DEF;
...
function Spark({ data, color }) {
  ...
  return <div style={{ position:"relative",overflow:"hidden" }}>
    ...
    <div style={{ position:"relative",zIndex:1,background:C.card,transform:`translateX(${off}px)`,transition:Math.abs(off) === 80 || off===0 ? "transform .18s" : "none" }}
      ...
      onTouchEnd={() => { if (off < -40) { setOff(-80); setConfirm(true); }
        else if (off > 40) { setOff(80); setTimeout(() => { onEdit?.(); reset(); }, 100); }
        else reset();
        sx.current = null;
      }}
  ...
  "🏠":["家居","房","租","水","電","瓦斯","網路","清潔","家具","修繕","管理","住","宿舍"],
  "🎬":["娛樂","電影","遊戲","ktv","旅遊","旅行","出遊","景點","門票","演唱會","展覽"],
  "📱":["訂閱","netflix","youtube","spotify","apple","google","軟體","app","會員","月費"], "💰":["薪資","薪水","工資","月薪","獎金","年薪","兼職"], "📖":["教育","學費","書","課程","補習","學習","培訓","證照"], "💊":["醫療","醫院","診所","藥","健康","保健","牙醫","健檢"], "💄":["美容","美髮","美甲","保養","化妝","髮廊","spa","美容院"], "👕":["衣","服","鞋","包包","配件","時尚","購物","服飾"], "🎓":["學費","大學","研究所","學校"], "🐾":["寵物","狗","貓","動物","飼料","獸醫"], "🏋️":["運動","健身","球","游泳","跑步","瑜珈","體育"], "✈️":["旅行","旅遊","出國","機票","飯店","住宿"], "🎁":["禮物","送禮","生日","紅包","祝賀","婚禮"], "💻":["電腦","3c","科技","手機","相機","耳機","設備"], "🏦":["利息","銀行","投資","理財","股息","股票","基金"], "🧴":["日用","生活用品","衛生","清潔用品"], "🍵":["飲料","咖啡","茶","手搖","飲品"], "🚗":["汽車","加油","保養","車險","停車費"], "📦":["其他","雜項","雜費","不確定"],
};
function guessEmoji(name) {
  const n = name.toLowerCase();
  for (const [emoji, keywords] of Object.entries(EMOJI_KEYWORDS)) {
...
}const pnlColor = (val, C) => val>0 ? C.income : val<0 ? C.expense : C.textSub;function InfoBtn({ msg }) {
  const [show, setShow] = useState(false);
  return (
    <span style={{ position:"relative", display:"inline-block" }}>
      <button onClick={e => { e.stopPropagation(); setShow(p=>!p); }}
...
══════════════════════════════════════════════════════ */
const MANUAL_DATA = [
  { key:"m_overview",icon:"📊",title:"總覽 — 每日記帳",steps:[
    "點右下角 ✏️ 新增一筆收入或支出",
    "選類別、填金額、選帳戶 → 帳戶餘額自動更新",
    "說明欄會記憶同類別的歷史輸入，方便快速帶入",
    "左滑記錄可刪除（帳戶餘額自動還原）；右滑可編輯",
    "代墊：開啟「代墊」後填朋友名字和金額，自動拆成兩筆（自己那份支出 + 代墊轉帳），並在往來帳建立應收",
    "分月認列：收到一次性大額收入可開啟，之後每月到「待認列收入池」手動認列",
  ]},
  { key:"m_wallet",icon:"👛",title:"錢包 — 帳戶管理",steps:[
    "右上角 ➕ 可新增：現金、金融卡、證券帳戶、信用卡、儲蓄保單",
    "帳戶分區：流動資產（現金/金融卡）、負債（信用卡）、非流動資產（證券）、儲蓄保單（追蹤用）",
    "點帳戶名稱 → 查看該帳戶所有交易細項",
    "右滑帳戶 → 編輯餘額、圖示、名稱；編輯餘額時可填調整說明",
    "信用卡：輸入應付金額，點 Pay 繳費時選扣款帳戶",
    "訂閱管理：設定固定扣款（每月/每週/每年），到期自動記帳（需重新開啟 App）",
    "基本開銷：水電費、房租等固定支出，同上自動記帳邏輯",
  ]},
  { key:"m_policy",icon:"🛡️",title:"儲蓄保單 — 保單追蹤",steps:[
    "點 ➕ → 選「儲蓄保單」新增（醫療險請放基本開銷）",
    "填保單名稱、保險公司、目前解約金、已繳總保費、幣別、起保日",
    "儲蓄保單不計入總資產，純追蹤損益用",
    "💰 繳保費：帳戶餘額自動扣除，已繳總保費自動累加",
    "✏️ 更新：每年收到對帳單後，更新解約金和已繳總保費",
    "📋 解約：填實際領回金額，自動記兩筆（收入+支出），損益立刻結算",
    "損益 = 解約金 − 已繳總保費（負數代表還沒回本）",
  ]},
  { key:"m_invest",icon:"📈",title:"投資 — 股票追蹤",steps:[
    "首次使用：點「📋 現有持股」登錄目前持有的股票，代號不要加 .TW",
    "之後買入：點「＋買入」，上方可快速選擇重複買入的股票",
    "賣出：點個股 → 賣出，下拉選擇持股，自動帶入市價損益",
    "市價開啟 App 時自動更新；點個股看損益、今日行情、三大法人（台股）",
    "技術線圖 → TradingView；個股新聞 → Google News（代號+名稱搜尋）",
    "停損提醒：在個股細項設虧損%門檻，達到時顯示 🔴 警示",
  ...
  { key:"m_notes",icon:"👥",title:"往來帳 — 借貸管理",steps:[
    "別人欠我 💚：記錄應收款；我欠別人 🟡：記錄應付款",
    "分期付款：可設 2-48 期，每期金額可手動修改",
    "收款/付款：點「收一期」或「付一期」，選帳戶，餘額自動更新",
    "應收收款不算收入（本來就是你的錢）；應付付款才算支出",
    "7 天內到期顯示提醒，逾期顯示紅色 ⚠️ 警告",
  ]},
  { key:"m_charts",icon:"🗂️",title:"圖表 — 收支分析",steps:[
    "圓餅圖：查看本月各類別支出/收入佔比",
    "收支健康度：自訂區間查看儲蓄率、支出佔比",
    "資產成長：預設本月，可切換多月；帳戶調整不計入",
    "投資圖表：資產配置（流動/非流動）、持股比例、投資成長",
    "目標管理：點「＋新增目標」設定金額、期限；不指定帳戶=用總資產淨值",
  ]},
  { key:"m_settings",icon:"⚙️",title:"設定 — 個人化",steps:[
    "主題：深色🌙 / 淺色☀️ / 紫色💜 / 海洋🌊，即時切換",
    "類別管理：點類別可改名稱和 emoji；可新增自訂類別",
    "匯出備份：定期點「📤 匯出備份」存成 JSON 檔，換裝置前必做",
    "匯入備份：點「📥 匯入備份」選檔案還原所有資料",
  ...
  { key:"m_tips",icon:"💡",title:"常見問題 & 小技巧",steps:[
    "Q：帳戶餘額沒更新？→ 記帳時確認有選帳戶",
    "Q：投資市價沒顯示？→ 重新開啟 App，等幾秒讓市價載入",
    "Q：訂閱沒自動記帳？→ 需要關掉重新開啟 App 才觸發",
    "Q：往來帳收款為什麼不算收入？→ 那是別人還你本來就是你的錢",
...
const LEARN_DATA = [{ section:"🌱 基礎觀念", key:"learn_basic",
  items:[
    { title:"什麼是複利？為什麼說是第八大奇蹟", tag:"入門", url:"https://rich01.com/?s=複利" },
    { title:"通貨膨脹是什麼？為什麼錢放著會變薄", tag:"入門", url:"https://rich01.com/?s=通貨膨脹" },
    { title:"資產與負債的差別，富人思維的起點", tag:"入門", url:"https://rich01.com/?s=資產負債" },
    { title:"緊急預備金要存多少才夠？", tag:"入門", url:"https://rich01.com/?s=緊急預備金" },
  ]
},{ section:"📈 股票投資", key:"learn_stock",
  items:[
    { title:"股票是什麼？買股票就是買公司的一部分", tag:"基礎", url:"https://rich01.com/?s=股票是什麼" },
    { title:"ETF 是什麼？為什麼適合一般投資人", tag:"基礎", url:"https://rich01.com/?s=ETF" },
    { title:"0050 vs 0056，哪個適合你？", tag:"台股", url:"https://rich01.com/?s=0050+0056" },
    { title:"定期定額投資法，降低進場時機風險", tag:"策略", url:"https://rich01.com/?s=定期定額" },
    { title:"股票的本益比（PE）怎麼看？", tag:"進階", url:"https://rich01.com/?s=本益比" },
  ]
},{ section:"🏦 資產配置", key:"learn_alloc",
  items:[
    { title:"資產配置是什麼？分散風險的核心概念", tag:"重要", url:"https://rich01.com/?s=資產配置" },
    { title:"股債配置：股票與債券的比例怎麼決定", tag:"策略", url:"https://rich01.com/?s=股債配置" },
    { title:"全球分散投資：為什麼不要只買台股", tag:"策略", url:"https://rich01.com/?s=全球分散" },
    { title:"懶人投資法：長期持有 ETF 的優缺點", tag:"策略", url:"https://rich01.com/?s=懶人投資" },
  ]
},{ section:"🛡️ 風險管理", key:"learn_risk",
  items:[
    { title:"投資風險有哪些？如何評估自己的風險承受度", tag:"重要", url:"https://rich01.com/?s=投資風險" },
    { title:"停損是什麼？設停損點的邏輯", tag:"策略", url:"https://rich01.com/?s=停損" },
    { title:"不要把雞蛋放在同一個籃子裡", tag:"基礎", url:"https://rich01.com/?s=分散投資" },
  ]
},{ section:"💰 理財規劃", key:"learn_plan",
  items:[
    { title:"50/30/20 法則：收入分配的簡單框架", tag:"入門", url:"https://rich01.com/?s=50+30+20" },
    { title:"財務自由是什麼？FIRE 運動介紹", tag:"目標", url:"https://rich01.com/?s=財務自由" },
    { title:"退休規劃：幾歲開始存才夠？", tag:"規劃", url:"https://rich01.com/?s=退休規劃" },
    { title:"保險怎麼買？先保障再儲蓄的原則", tag:"規劃", url:"https://rich01.com/?s=保險" },
  ]
}];
...
function App() {
  ...
  const { accs, txns, debts, subs, bills, stocks, pools, cats, rates, goals, policies } = d;  const [tab, setTab] = useState("overview");
  const [theme, setTheme] = useState(() => localStorage.getItem("finzen_theme") || "dark");
  C = getC(theme);
  iSt = getISt();
  const changeTheme = (t) => { localStorage.setItem("finzen_theme", t); setTheme(t); };
  ...
  const confirm = (msg, onOk) => setConfirmDlg({ msg, onOk });
  const closeConfirm = () => setConfirmDlg(null);
  const close = () => setModal(null);  const [selTxn, setSelTxn] = useState(null);
  const [selAcc, setSelAcc] = useState(null);
  const [selStock, setSelStock] = useState(null);
  const [selSub, setSelSub] = useState(null);
  const [selBill, setSelBill] = useState(null);
  const [selPool, setSelPool] = useState(null);  const [wMode, setWMode] = useState("normal");
  const [collapsed, setCollapsed] = useState({});
  const toggleSection = key => setCollapsed(p => ({ ...p, [key]: !p[key] }));
  const [useMvForAssets, setUseMvForAssets] = useState(() => localStorage.getItem("finzen_useMv") === "true");
  const toggleMv = () => { const v = !useMvForAssets; setUseMvForAssets(v); localStorage.setItem("finzen_useMv", String(v)); };
  const [tooltip, setTooltip] = useState(null); // {msg, x, y}  const [month, setMonth] = useState(() => { const d = new Date(); return { y:d.getFullYear(), m:d.getMonth() + 1 }; });
  const [chartRange, setChartRange] = useState(() => { const now = TODAY.slice(0,7); return { s:`${now}-01`, e:TODAY }; });
  const [healthRange, setHealthRange] = useState(() => { const d = new Date(); return { s:`${d.getFullYear()}-${String(d.getMonth() + 1).padStart(2, "0")}-01`, e:TODAY }; });
  const [showDP, setShowDP] = useState(false);
  const [showHDP, setShowHDP] = useState(false);  const [invTab, setInvTab] = useState("holdings");
  const [invPie, setInvPie] = useState("alloc");
  const [mkt, setMkt] = useState("ALL");
  const [fetchingTicker, setFetchingTicker] = useState(false);
  const [tickerPreview, setTickerPreview] = useState(null);  const [sq, setSq] = useState(""), [showSq, setShowSq] = useState(false);
  const [chartView, setChartView] = useState("expense");
  const [newBal, setNewBal] = useState("");
  const [adjDesc, setAdjDesc] = useState("");
  const [newCatType, setNewCatType] = useState("expense");
  ...
  const [editGoal, setEditGoal] = useState(null);
  const [settleAcc, setSettleAcc] = useState("");
  const [settleCustomAmt, setSettleCustomAmt] = useState(null);  const T0 = { type:"expense",cat:"食物",amt:"",desc:"",acc:"",date:TODAY,tags:"",proxy:false,proxyList:[{ person:"",amt:"" }],deferred:false,deferMonths:"4",deferMoAmt:"" };
  const [nT, setNT] = useState(T0);
  const D0 = { type:"receivable",person:"",amt:"",desc:"",date:TODAY,note:"",installTotal:0,installAmt:"",installPaid:0,installPaidAmt:0 };
  const [nD, setND] = useState(D0);
  const S0 = { name:"",amt:"",acc:"",day:"1",weekday:"1",freq:"month",cat:"訂閱",period:"month",periodN:"1" };
  ...
  const [buyF, setBuyF] = useState(BF0);
  const [sellF, setSellF] = useState({ stockId:"",shares:"",totalProceeds:"",fee:"",pnl:"",pnlType:"income",returnAcc:"" });
  const [payF, setPayF] = useState({ creditId:"",fromId:"",amt:"",date:TODAY,note:"" });  useEffect(() => { if (!updateMsg) return; }, [updateMsg]);
  useEffect(() => { const t = setInterval(() => { try { localStorage.setItem("finzen_backups", JSON.stringify([{ ts:Date.now(), data:d }])); } catch {} }, 5 * 60 * 1000); return () => clearInterval(t); }, [d]);  const fetchPrice = useCallback(async (ticker, market) => {
    // 美股：直接用 Yahoo Finance
    const sym = market ==="TW" ? `${ticker}.TW` : ticker;
    const yahooV7 = `https://query1.finance.yahoo.com/v7/finance/quote?symbols=${sym}&fields=regularMarketPrice,shortName,longName`;
    const yahooV8 = `https://query2.finance.yahoo.com/v8/finance/chart/${sym}?interval=1d&range=2d`;
  ...
  const fetchAllPrices = useCallback(async (stockList) => {
  ...
  }, [stocks, fetchPrice, upd]);  useEffect(() => {
    if (!d || !d.subs) return;
    const getDueDates = (item, lastDate) => {
      const dates = [];
      const today = new Date(TODAY);
      // 如果有 lastDate，從 lastDate 隔天開始
      const start = lastDate ? new Date(lastDate) : (() => {
        const d = new Date(item.date || TODAY);
        d.setDate(d.getDate() - 1); // 減1天讓後續邏輯能包含設定當天
        return d;
      })();
      if (item.freq ==="week") {
    ...
    (d.bills ||[]).filter(b => b.active).forEach(b => {
      ...
      const dues = getDueDates(b, lastDate);
      dues.forEach(date => {
        newTxns.push({ id: Date.now() + Math.random(), type: "expense", cat: b.cat || "家居", amt: b.amt, desc: b.name, acc: b.acc ||"", date, tags: "#自動記帳", autoSrc: b.id,
        });
      });
      if (dues.length>0) {
        upd("bills", p => p.map(x => x.id===b.id ? { ...x, lastBilled: dues[dues.length - 1] } : x));
  ...
  const prevMo = () => setMonth(({ y, m }) => m === 1 ? { y:y - 1, m:12 } : { y, m:m - 1 });
  const nextMo = () => setMonth(({ y, m }) => m === 12 ? { y:y + 1, m:1 } : { y, m:m + 1 });
  const filtWL = useMemo(() => mkt === "ALL" ? [] : [], [mkt]);  const addTxn = () => {
    if (!nT.amt) return;
    const id = Date.now();
    const validProxies = nT.proxy ? nT.proxyList.filter(p => p.person && +p.amt>0) : [];
    const totalProxyAmt = validProxies.reduce((s, p) => s + +p.amt, 0);
    ...
    if (validProxies.length>0) {
      // 筆1：自己的支出（只算自己那份）
      const ownTxn = { ...nT, id, amt:ownAmt, proxyAmt:0, proxyFor:"", proxyList:[], desc:nT.desc || nT.cat };
      const proxyTxn = { ...nT, id:id+1, type:"transfer", cat:"往來帳", amt:totalProxyAmt, proxyAmt:totalProxyAmt, proxyFor:validProxies.map(p => p.person).join("、"), proxyList:validProxies,
        desc:"代墊：" + (nT.desc || nT.cat) + "（" + validProxies.map(p => p.person + " " + fmt(+p.amt)).join("、") + "）", tags:"#代墊",
      };
      upd("txns", p => [...p, ownTxn, proxyTxn]);
      const acc = accs.find(a => a.name===nT.acc);
      if (acc) {
  ...
  const saveTxn = t => {
    ...
    if (old) {
      upd("accs", p => {
        let updated = [...p];
        if (old.type ==="income" && old.acc) {
          updated = updated.map(a => a.name===old.acc ? {...a, bal:a.bal - old.amt} : a);
        } else if (old.type ==="expense" && old.cat !== "帳戶調整") {
          const oldAcc = updated.find(a => a.name===old.acc);
          if (oldAcc?.type ==="credit") updated = updated.map(a => a.name===old.acc ? {...a, payable:Math.max(0,(a.payable||0)-old.amt)} : a);
          else if (old.acc) updated = updated.map(a => a.name===old.acc ? {...a, bal:a.bal + old.amt} : a);
        }
        if (t.type ==="income"&&t.acc) {
          updated = updated.map(a => a.name===t.acc ? {...a, bal:a.bal + t.amt} : a);
        } else if (t.type ==="expense"&&t.cat!=="帳戶調整") {
          const newAcc = updated.find(a => a.name===t.acc);
          if (newAcc?.type ==="credit") updated = updated.map(a => a.name===t.acc ? {...a, payable:(a.payable||0)+t.amt} : a);
          else if (t.acc) updated = updated.map(a => a.name===t.acc ? {...a, bal:a.bal - t.amt} : a);
        }
        return updated;
      });
    }
    upd("txns", p => p.map(x => x.id===t.id ? t : x));
    close();
  };
  const adjBal = (acc, newBalStr, isFirst, adjDesc) => {
    const desc = adjDesc ||"";
  ...
  const doTransfer = () => {
    const a = +trAmt; if (!a || !trFrom || !trTo || trFrom === trTo) return;
    const fromAcc = accs.find(ac => ac.id === trFrom);
    const toAcc = accs.find(ac => ac.id === trTo);
    upd("accs", p => p.map(ac => { if (ac.id === trFrom) return { ...ac, bal:ac.bal - a }; if (ac.id === trTo) return { ...ac, bal:ac.bal + a }; return ac; }));
    upd("txns", p => [...p, { id: Date.now(), type:"transfer", cat:"帳戶調整",
      amt: a, desc: `帳戶轉帳`, acc: fromAcc?.name||"", toAcc: toAcc?.name||"",
      date: TODAY, tags:"#轉帳"
    }]);
    setTrFrom(""); setTrTo(""); setTrAmt(""); close();
  };
  ...
  const doBuy = () => {
    ...
    const ex = stocks.find(s => s.ticker===buyF.ticker && s.acc===buyF.acc);
    if (ex) {
      upd("stocks", p => p.map(s => s.id===ex.id ? { ...s, name: buyF.name || s.name, trades: [...s.trades, trade],
      } : s));
    } else {
      upd("stocks", p => [...p, { id:"s"+Date.now(), acc:buyF.acc,
        ticker:buyF.ticker, name:buyF.name||buyF.ticker, market:buyF.market, curPrice: 0, manualShares: null, manualAvgCost: null, manualTotalCost: null, trades:[trade],
      }]);
    }
    if (buyF.fromAcc && buyF.totalCost) {
      const cost = +buyF.totalCost;
      upd("accs", p => p.map(a => a.name===buyF.fromAcc ? {...a, bal:a.bal-cost} : a));
      if (buyF.acc) upd("accs", p => p.map(a => a.name===buyF.acc ? {...a, bal:a.bal+cost} : a));
      upd("txns", p => [...p, { id:Date.now(), type:"transfer", cat:"帳戶調整", amt:cost,
        desc:`買入 ${buyF.ticker} ${buyF.shares}股(均${buyF.avgCost||0}元)`, acc:buyF.fromAcc, toAcc:buyF.acc||"", date:TODAY, tags:"#投資",
      }]);
    }
    setBuyF(BF0); close();
  };
  const doSell = () => {
    ...
    const sellPrice = +sellF.shares>0 && proceeds>0 ? proceeds / +sellF.shares : 0;
    const sellFee = sellF.fee ? +sellF.fee : 0;
    upd("stocks", p => p.map(s => s.id===st.id ? { ...s,
      trades:[...s.trades, { id:"t"+Date.now(), type:"sell", date:TODAY, shares:+sellF.shares, price:sellPrice, fee:sellFee, totalProceeds:proceeds }],
    } : s));
    if (sellF.returnAcc && proceeds) {
      upd("accs", p => p.map(a => a.name===sellF.returnAcc ? {...a, bal:a.bal+proceeds-sellFee} : a));
      upd("accs", p => p.map(a => a.name===st.acc ? {...a, bal:Math.max(0, a.bal-proceeds)} : a));
      upd("txns", p => [...p, { id:Date.now(), type:"transfer", cat:"帳戶調整", amt:proceeds, desc:`賣出 ${st.ticker} ${sellF.shares}股`, acc:st.acc, toAcc:sellF.returnAcc, date:TODAY, tags:"#投資",
      }]);
    }
    if (pnlAmt>0) upd("txns", p => [...p, { id:Date.now()+1, type:isProfit?"income":"expense", cat:"投資收益", amt:pnlAmt,
      desc:`${isProfit?"投資獲利":"投資虧損"}：${st.ticker} ${sellF.shares}股`, acc:sellF.returnAcc||"", date:TODAY, tags:"#投資",
    }]);
    setSellF({ stockId:"", shares:"", totalProceeds:"", fee:"", pnl:"", pnlType:"income", returnAcc:"" }); close();
  };
  const doRecognize = () => {
    ...
    <>
      ...
      <div style={{ maxWidth:480,margin:"0 auto", minHeight:"100dvh",background:C.bg,color:C.text, fontFamily:"'Noto Sans TC',system-ui,sans-serif", display:"flex",flexDirection:"column" }}>
          ...
          <div style={{ display:"flex",gap:6 }}>
            {[{ icon:"👁", mode:"vis" }, { icon:"⠿", mode:"sort" }, { icon:"➕", cb:() => setModal("addAccType") }].map((b, i) => (
              <button key={i} onClick={b.cb || (() => setWMode(p => p === b.mode ? "normal" : b.mode))}
                style={{ width:36,height:36,borderRadius:10,
                  background: b.mode && wMode === b.mode ? `${C.accent}40` : `${C.text}12`,
                  border:"1px solid " + (b.mode && wMode === b.mode ? C.accent : C.text + "30"), cursor:"pointer", color: b.mode && wMode === b.mode ? C.accent : C.text,
                  fontSize:16, display:"flex", alignItems:"center", justifyContent:"center" }}>
                {b.icon}
              </button>
            ))}
        ...
        return <Card key={g.id} style={{ padding:20,marginBottom:12,border:`1px solid ${pct>=100?C.teal:C.border}` }}>
          ...
          {remaining>0 && <div style={{ marginTop:6,fontSize:12,color:C.muted,textAlign:"center" }}>還差 <strong style={{ color:col }}>{fmt(remaining)}</strong></div>}
          {/* 里程碑鼓勵 */}
          {(() => {
            const msgs = [
              { min:100, emoji:"🎉🏆", text:"目標達成！你真的太厲害了，繼續保持！" }, { min:95,  emoji:"🔥",   text:"差一點點就到了！衝刺！" }, { min:90,  emoji:"💪",   text:"90% 了！終點在眼前，加油！" }, { min:80,  emoji:"⭐",   text:"超過八成！你已經超越大多數人了！" }, { min:70,  emoji:"🚀",   text:"70% 達成！保持這個節奏！" }, { min:50,  emoji:"🎯",   text:"過半了！上半場漂亮，繼續！" }, { min:30,  emoji:"🌱",   text:"穩定進步中，方向正確！" }, { min:20,  emoji:"👏",   text:"有個好的開始！繼續累積！" },
            ];
            const m = msgs.find(m => pct >= m.min);
            if (!m) return null;
            const color = pct >= 100 ? C.teal : pct >= 80 ? C.income : col;
            return <div style={{ marginTop:8,padding:"8px 12px",borderRadius:10,background:`${color}15`,textAlign:"center" }}>
              <span style={{ fontSize:16 }}>{m.emoji}</span>
              <span style={{ fontSize:12,fontWeight:700, color, marginLeft:6 }}>{m.text}</span>
            </div>;
          })()}
        </Card>;
              })}
        ...
        {!collapsed["manual"] && MANUAL_DATA.map((item) => (
          <div key={item.key} style={{ borderBottom:`1px solid ${C.border}`,marginBottom:4 }}>
            <button onClick={() => toggleSection(item.key)}
              style={{ display:"flex",alignItems:"center",justifyContent:"space-between",width:"100%",background:"none",border:"none",cursor:"pointer",padding:"10px 0" }}>
              <div style={{ display:"flex",alignItems:"center",gap:8 }}>
        ...
        const desc = isInstall ? (isReceivable ? `分期收款 ${newPaidCount}/${d.installTotal}` : `分期付款 ${newPaidCount}/${d.installTotal}`) + `：${d.person} ${d.desc||""}` : (isReceivable ? "應收款結清" : "應付款結清") + `：${d.person} ${d.desc||""}`;
        upd("txns", p => [...p, { id:Date.now(), type: isReceivable ? "transfer" : "expense", cat: "往來帳", amt: thisPay, desc, acc: settleAcc ||"", toAcc: isReceivable ? settleAcc : "", date: TODAY, tags: "#往來帳",
        }]);
        setSettleDebt(null); setSettleAcc(""); setSettleCustomAmt(null); close();
              }}>✓ {isInstall ? (isReceivable?`收第${newPaidCount}期`:`付第${newPaidCount}期`) : (isReceivable?"確認收款":"確認付款")}</Btn>
              <Btn v="secondary" style={{ flex:1 }} onClick={close}>取消</Btn>
        ...
        upd("txns", p=>[...p,
          { id:now, type:"transfer", cat:"往來帳", amt:totalPaidTWD,
            desc:selPolicy.name + " 解約 — 本金回收" + (isForeign ? " (" + selPolicy.cur + ")" : ""), acc:"", toAcc:surrenderAcc||"", date:TODAY, tags:"#保單" }, ...(pnl!==0 ? [{ id:now+1, type: pnl>0 ? "income" : "expense", cat: pnl>0 ? "投資收益" : "其他", amt: Math.abs(pnl),
            desc:selPolicy.name + " 解約 — " + (pnl>0?"獲利":"虧損") + (isForeign ? " (" + selPolicy.cur + "換算)" : ""),
            acc: surrenderAcc||"", date:TODAY, tags:"#保單" }] : []),
        ]);
        upd("policies", p=>p.filter(x=>x.id!==selPolicy.id));
        ...
        const ex = p.find(s => s.ticker === buyF.ticker && s.acc === buyF.acc);
        if (ex) {
          return p.map(s => s.id===ex.id ? { ...s, name: buyF.name || s.name, manualShares: +buyF.shares,
            manualAvgCost: buyF.avgCost ? +buyF.avgCost : s.manualAvgCost,
            manualTotalCost: buyF.totalCost ? +buyF.totalCost : s.manualTotalCost,
          } : s);
        }
        return [...p, { id: "s"+Date.now(), acc:buyF.acc, ticker:buyF.ticker, name:buyF.name||buyF.ticker, market:buyF.market, curPrice:0, manualShares: +buyF.shares, manualAvgCost: buyF.avgCost ? +buyF.avgCost : null,
          manualTotalCost: buyF.totalCost ? +buyF.totalCost : null, trades: [],
        }];
              });
              if (buyF.acc && buyF.totalCost) {
        upd("accs", p => p.map(a => a.name===buyF.acc ? { ...a, bal: a.bal + +buyF.totalCost } : a));
        ...
        <div style={{ padding:12,borderRadius:12,background:`${C.accent}12`,border:`1px solid ${C.accent}44`,marginBottom:10 }}>
          ...
          <div style={{ display:"flex",gap:6 }}>
            <button onClick={() => {
              ...
              const oldName = editCat.oldName;
              upd("cats", p => ({ ...p, [type]:p[type].map(c => c===oldName ? newName : c) }));
              addCustomCE(newName, editCat.emoji);
              if (newName !== oldName) {
                upd("customCE", p => { const n = {...(p||{})}; delete n[oldName]; n[newName] = editCat.emoji; return n; });
                upd("txns", p => p.map(t => t.cat===oldName ? {...t, cat:newName} : t));
                upd("subs", p => p.map(s => s.cat===oldName ? {...s, cat:newName} : s));
                upd("bills", p => p.map(b => b.cat===oldName ? {...b, cat:newName} : b));
              } else {
                upd("customCE", p => ({ ...(p||{}), [newName]:editCat.emoji }));
              }
              setEditCat(null);
            }} style={{ flex:1,padding:"8px",borderRadius:10,background:C.accent,color:"#fff",border:"none",fontWeight:700,cursor:"pointer" }}>儲存</button>
            <button onClick={() => {
              const fallback = type==="expense" ? "其他" : "其他收入";
              upd("txns", p => p.map(t => t.cat===editCat.oldName ? {...t, cat:fallback} : t));
              upd("subs", p => p.map(s => s.cat===editCat.oldName ? {...s, cat:fallback} : s));
              upd("bills", p => p.map(b => b.cat===editCat.oldName ? {...b, cat:fallback} : b));
              upd("cats", p => ({ ...p, [type]:p[type].filter(c => c!==editCat.oldName) }));
              upd("customCE", p => { const n = {...(p||{})}; delete n[editCat.oldName]; return n; });
              setEditCat(null);
            }} style={{ padding:"8px 14px",borderRadius:10,background:`${C.danger}22`,color:C.danger,border:`1px solid ${C.danger}44`,cursor:"pointer",fontWeight:700 }}>刪除</button>
            <button onClick={() => setEditCat(null)}
              style={{ padding:"8px 12px",borderRadius:10,background:C.card,color:C.muted,border:`1px solid ${C.border}`,cursor:"pointer" }}>✕</button>
